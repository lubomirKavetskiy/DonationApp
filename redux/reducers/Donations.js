...
const Donations = createSlice({
  ...
  reducers: {
    updateSelectedDonationId: (state, action) => {
      ...
      state.selectedDonationId = donationId;
      state.selectedDonationInformation = state.items.find(
        ({donationItemId}) => donationItemId === donationId,
      );
    },
    resetDonationsState: () => initialState,
  },
});

export const {updateSelectedDonationId, resetDonationsState} =
  Donations.actions;
